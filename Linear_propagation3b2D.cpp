...
const int nombreEvenement=3900;
int evenementControl=1, tester=0; // à combien on commence,
double Rang =2.38;
int perc=1.0, repetition, rep_max=1;
double vitesse=-2.0, perc_moyen=0.0; // nombre moyen d'individu perçu
vector<double> perc_t; // vecteur nous permettant d'update le h

int ActivationMovers=1; //1 pour activer l'écriture du fichier movers
...
double Positions[10000][2], Positions_temp[10000];
...
int main(int argc, const char * argv[])
{
    ...
    for(repetition=0; repetition<rep_max; repetition++)
    {
        ...
        for (i=0; i<N; i++) // initialisation des positions
        {
            ...
            Positions[i][1]=i-(j*22);
            Positions[i][0]=j;
        }*/
        k=0;
        for (i=0; i<largeur; i++) // initialisation des positions en disque
        {
            for(j=0;j<largeur;j++){
                if(sqrt(pow(i-largeur/2,2)+pow(j-largeur/2,2))<largeur/2){
                    Positions[k][1]=i;
                    Positions[k][0]=j;
                    k++;//cout << k-1 << " " << Positions[k-1][0] << " " << Positions[k-1][1] << endl;
                }
            }
        }
        N=k;cout << k << endl;
        ...
        Movers.clear();
        MoversPos.clear();
        Movers.push_back(10); // La liste des individus en déplacement contient au moins le premier i.e. 0
        last_front_mover=0; // Le dernier en mouvement de la ligne est 0 au départ
        front_mover=0; // même raison
        first_front_static=1; // Le premier statique est forcément 1
	    ...
	    perc_t.clear();
            ...
            for(i=0;i<N;i++){
                toMovers << Positions[i][0] << " " << Positions[i][1] << " " << Etats[i] << endl;
            }
            toMovers << endl;
            ...
            if (ActivationMovers==1)
            {
                ...
                {
                    MoversPos_temp.push_back(Positions[*it][0]);
                }
            ...
            delta=-Positions[last_front_mover][0]+Positions[first_front_static][0];
            toDelta << t << "\t \t";
            toDelta << delta << endl;
...
void Move(double temps, int individu)
{
    ...
    Positions[individu][0]+=(vitesse*temps);
    //cout << Positions << endl;
...
{
    double dist;
    dist=sqrt(pow(Positions[i][0]-Positions[j][0],2)+pow(Positions[i][1]-Positions[j][1],2));
    return dist;
}
