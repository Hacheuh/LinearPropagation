...
double Rang =1.53;
int perc=2, repetition, rep_max=20;
double vitesse=900.0, perc_moyen=0.0; // nombre moyen d'individu perçu
...
int main(int argc, const char * argv[])
{
  ...
  for(repetition=0; repetition<rep_max; repetition++)
  {
    ...
    double t = 0.0, h = 0.1/(abs(vitesse)*pow(Rang,betaCA));
    ...
    first_front_static=0+1; // Le premier statique est forcément 1
    
    int last_front_moverL=0; // Le dernier en mouvement de la ligne est 0 au départ côté gauche
    int front_moverL=0; // même raison
    int first_front_staticL=0-1; // Le premier statique à gauche est forcément -1
    ...
    while (t < T)
    {
      ...
      if (ActivationMovers==1)
      {//Movers temps positions etats
      
        for(i=0;i<N;i++){
            toMovers << t << " " << Positions[i] << " " << Etats[i] << endl;
        }
        toMovers << endl;
      }
...
double ComptageEtat(int etat, vector<int> liste, int quest, int indiv_focus)
{
  ...
  else if(quest==1)                      //calcul du nombre d'état de manière controlé, pour le calcul des proba indiv
  {
    ...
    // Quorum sensing
    //nombreEtat=pow(nombreEtat,3)/(1+pow(nombreEtat,3));
